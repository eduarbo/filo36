#!/usr/bin/env python3
"""Pack unchanged revF meshes for the interactive viewer. Millimetres throughout.
...
scene = {'revision':'F', 'units':'mm', 'geometries':{}, 'parts':[], 'sources':[]}
groups = {
    ...
    'battery-retainer':('Retenedor propuesto','supports','#78908a',24),
...
def mesh(path):
    ...
    indices=cells[:,[1,2,3] if path.startswith('mechanical/revF/') else [1,3,2]].reshape(-1)
...
for side,keys in layout['halves'].items():
    ...
    for name,(label,group,color,explode) in groups.items():
        add(label,side,group,color,[offset,0,0],explode,mesh(f'mechanical/revF/{side}-{name}.stl'))
    for i in range(1,6):
        add(f'Arandela {i}',side,'fasteners','#89918a',[offset,0,0],9,mesh(f'mechanical/revF/{side}-washer-{i}.stl'))
    for key in keys:
    ...
    add('LCD · contenido ilustrativo',side,'display','#c5d1ba',[offset+cx,16.12,33.8],52,
...
for path in ['design/revF.json','design/layout.json','tools/build_viewer_revF.py']:
    scene['sources'].append({'path':path,'sha256':digest(path)})
