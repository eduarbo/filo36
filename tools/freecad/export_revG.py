"""Export the active revG FCStd, preserving manual changes in the source document.
...
OUT=ROOT/'mechanical/revG'
doc=A.ActiveDocument
if not doc or not doc.getObject('Parameters'):
    raise RuntimeError('Open mechanical/revG/Filo36.FCStd first.')
doc.recompute()
metadata=json.loads((ROOT/'design/revG.json').read_text())
...
for side,prefix in [('left','L_'),('right','R_')]:
    ...
    report['halves'][side]={'frame_variants':frame_checks,'pair_intersections_mm3':pairs,'collisions':issues,
...
metadata['inputs']=[{'path':p,'sha256':hashlib.sha256((ROOT/p).read_bytes()).hexdigest()} for p in [
    'design/layout.json','design/revF-profiles.json','design/revG-frame-profiles.json','keycaps/catalog.json','tools/freecad/build_revG.py','tools/freecad/export_revG.py']]
metadata['fcstd_sha256']=hashlib.sha256((OUT/'Filo36.FCStd').read_bytes()).hexdigest()
(ROOT/'design/revG.json').write_text(json.dumps(metadata,indent=2)+'\n')
(ROOT/'validation/revG-mechanical.json').write_text(json.dumps(report,indent=2)+'\n')
assert not any(v['collisions'] for v in report['halves'].values()),'Unresolved nominal intersections; see validation/revG-mechanical.json'
