...
Only temporary copies under build/revH are saved.
...
G.showMainWindow();doc=A.openDocument(str(ROOT/'mechanical/revH/Filo36.FCStd'));doc.recompute()
sourcehash=hashlib.sha256((ROOT/'mechanical/revH/Filo36.FCStd').read_bytes()).hexdigest()
opaque_samples=0
...
path=ROOT/'build/revH/customized.FCStd';doc.saveAs(str(path));A.closeDocument(doc.Name)
...
assert hashlib.sha256((ROOT/'mechanical/revH/Filo36.FCStd').read_bytes()).hexdigest()==sourcehash
report={'source_sha256':sourcehash,'native_features_no_custom_proxy':True,'configuration_roundtrip':True,'reopened_customized_file':True,'all_36_key_centres_unchanged':True,'stem_tip_datum_mm':11.7,'FrameTop_edit_mm':[16.6,17.2],'preset':'saddle-sculpted','thumb_variant':'MX-size Normal 90deg','frames':['handheld','tv'],'invalid_configuration_rejected_atomically':True,'xy_margin_mm':margin,'source_file_unchanged':True,'opaque_side_wall_samples':opaque_samples}
(ROOT/'validation/revH-freecad.json').write_text(json.dumps(report,indent=2)+'\n')
