"""Export the active revH FCStd, preserving manual changes in the source document.
...
if not doc or not doc.getObject('Parameters'):
    raise RuntimeError('Open mechanical/revH/Filo36.FCStd first.')
doc.recompute()
metadata=json.loads((ROOT/'design/revH.json').read_text())
...
metadata['inputs']=[{'path':p,'sha256':hashlib.sha256((ROOT/p).read_bytes()).hexdigest()} for p in [
    'design/layout.json','design/revH-profiles.json','design/revH-frame-profiles.json','keycaps/catalog.json','tools/freecad/build_revH.py','tools/freecad/export_revH.py']]
...
(ROOT/'validation/revH-mechanical.json').write_text(json.dumps(report,indent=2)+'\n')
assert not any(v['collisions'] for v in report['halves'].values()),'Unresolved nominal intersections; see validation/revH-mechanical.json'
