...
Only temporary copies under build/revG are saved.
...
G.showMainWindow();doc=A.openDocument(str(ROOT/'mechanical/revG/Filo36.FCStd'));doc.recompute()
sourcehash=hashlib.sha256((ROOT/'mechanical/revG/Filo36.FCStd').read_bytes()).hexdigest()
opaque_samples=0
...
nextconfig['frames']['left']={'style':'facet','color':'#ad7656'}
nextconfig['frames']['right']={'style':'smooth','color':'#596c7a'}
...
path=ROOT/'build/revG/customized.FCStd';doc.saveAs(str(path));A.closeDocument(doc.Name)
doc=A.openDocument(str(path));doc.recompute();assert extract(doc)==nextconfig
for prefix in ['L_','R_']:assert abs(doc.getObject(prefix+'ActiveFrame').Shape.BoundBox.ZMax-17.2)<1e-6
assert hashlib.sha256((ROOT/'mechanical/revG/Filo36.FCStd').read_bytes()).hexdigest()==sourcehash
report={'source_sha256':sourcehash,'native_features_no_custom_proxy':True,'configuration_roundtrip':True,'reopened_customized_file':True,'all_36_key_centres_unchanged':True,'stem_tip_datum_mm':11.7,'FrameTop_edit_mm':[16.6,17.2],'preset':'saddle-sculpted','thumb_variant':'MX-size Normal 90deg','frames':['facet','smooth'],'invalid_configuration_rejected_atomically':True,'xy_margin_mm':margin,'source_file_unchanged':True,'opaque_side_wall_samples':opaque_samples}
(ROOT/'validation/revG-freecad.json').write_text(json.dumps(report,indent=2)+'\n')
