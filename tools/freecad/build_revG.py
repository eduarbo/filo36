"""Build the editable revG study using only native FreeCAD features.
...
doc = A.newDocument('Filo36_revG')
doc.Label = 'Filo36 · revG · editable study'
...
frame_profiles=json.loads((ROOT/'design/revG-frame-profiles.json').read_text())
...
finals = {}
metadata = {'revision': 'G', 'units': 'mm', 'coordinate_system': 'FreeCAD X=KiCad X, Y=-KiCad Y; PCB top=5.4 mm',
...
for side in ('left', 'right'):
    ...
    styles={}
    for style,label in [('smooth','Smooth'),('bevel','Beveled'),('facet','Faceted')]:
        sections=[]
        for j,(z,contour) in enumerate([(6.3,'outer'),(15.4,'outer'),(16.6,'outer' if style=='smooth' else style)]):
...
exec(compile((ROOT/'tools/freecad/export_revG.py').read_text(),str(ROOT/'tools/freecad/export_revG.py'),'exec'))
