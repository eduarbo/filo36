...
ROOT=Path(__file__).resolve().parents[2];G.showMainWindow();doc=A.openDocument(str(ROOT/'mechanical/revI/Flan36.FCStd'))
def log(*x):sys.__stdout__.write(' '.join(map(str,x))+'\n');sys.__stdout__.flush()
for p in ['L_','R_']:doc.getObject(p+'Half').Placement=A.Placement()
doc.recompute()
report={'source_sha256':hashlib.sha256((ROOT/'mechanical/revI/Flan36.FCStd').read_bytes()).hexdigest(),'checker_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'scope':'Read-only left-half affected-pair nominal height study; no source save, contact qualification, tolerance or release-path acceptance','current':{},'candidates':{}}
...
for db,top in [(12.4,14.8),(12.4,15.6)]:
 ...
 log(top,hits)
assert hashlib.sha256((ROOT/'mechanical/revI/Flan36.FCStd').read_bytes()).hexdigest()==report['source_sha256']
