...
# Sixteen intentional corners, instead of inheriting the wing/notch and small
# incidental vertices. All key centers, angles and mounting centers are fixed.
controls=[[16.9,18],[36,18],[36,6],[54,6],[54,1],[78,1],[78,6],
          [111,6],[111,11],[135,11],[135,87],[127,95],[102,85],
          [70,85],[45,75],[16.9,75]]
outer,arcs=rounded(controls,1.2);shell=Polygon(outer);assert shell.is_valid
hood,h_arcs=rounded([[111,11],[135,11],[135,67],[111,67]],1.2)
frame=Polygon(hood)
coords=lambda p:[list(v) for v in p.exterior.coords][:-1]
profiles={};frames={}
left={'outer':outer,'outer_arcs':arcs,'control_vertices':controls,'corner_radius_mm':1.2,
      'inner':coords(shell.buffer(-1.8,join_style=2)),
      'pcb_outline':coords(shell.buffer(-2.15,join_style=2)),
...
(ROOT/'design/revH-profiles.json').write_text(json.dumps(profiles,indent=2)+'\n')
(ROOT/'design/revH-frame-profiles.json').write_text(json.dumps(frames,indent=2)+'\n')
print('16 case corners at R1.2; straight inner flank X135 / X25; matching cavity, plate and PCB')
