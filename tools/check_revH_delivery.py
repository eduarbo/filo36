...
assert sha('mechanical/revH/Filo36.FCStd')==m['fcstd_sha256']
for source in m['inputs']:assert sha(source['path'])==source['sha256'],source
for s in ['left','right']:
    for name,p in m['parts'].items():assert sha('mechanical/revH/'+name+'.stl')==p['stl_sha256'],name
mechanical=json.loads((ROOT/'validation/revH-mechanical.json').read_text())
...
render=json.loads((ROOT/'validation/revH-render.json').read_text());assert render['model_sha256']==sha('design/revH.json')
assert render['renderer_sha256']==sha('tools/render_revH.py')
for item in render['meshes']:assert sha(item['path'])==item['sha256']
for view,item in render['views'].items():assert sha(f'docs/images/revH-{view}.png')==item['image_sha256']
f=json.loads((ROOT/'validation/revH-frames-render.json').read_text());assert f['image_sha256']==sha('docs/images/revH-frames.png')
assert f['renderer_sha256']==sha('tools/render_frames.py')
for item in f['sources']:assert sha(item['path'])==item['sha256']
assert 'docs/images/revH-assembled.png' in (ROOT/'README.md').read_text()
...
native=json.loads((ROOT/'validation/revH-freecad.json').read_text());assert native['source_sha256']==sha('mechanical/revH/Filo36.FCStd')
outline=json.loads((ROOT/'validation/revH-outline.json').read_text())
for path,digest in outline['inputs'].items():assert sha(path)==digest,path
caps=json.loads((ROOT/'validation/revH-keycaps.json').read_text())
assert caps['catalog_sha256']==sha('keycaps/catalog.json') and caps['layout_sha256']==sha('design/layout.json')
electrical=json.loads((ROOT/'validation/revH-electrical.json').read_text());assert electrical['fabrication_ready'] is False
for side,half in electrical['halves'].items():
    assert half['pcb_sha256']==sha(f'hardware/revH/filo36-{side}.kicad_pcb')
    assert half['46_footprints_nets_drills_uuid_models_unchanged'] and half['locked_original_keys']==18
# Native App::Link delegates FrameStyle, so the 12 bodies plus 2 active links are sampled.
assert native['opaque_side_wall_samples']==168 and native['configuration_roundtrip']
assert sha('docs/parts.md')==json.loads((ROOT/'validation/revH-parts-links.json').read_text())['document_sha256']
