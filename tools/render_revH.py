#!/usr/bin/env python3
"""Render generated revH CAD and unchanged KLP meshes. No retouching or geometry edits.
...
m=json.loads((ROOT/'design/revH.json').read_text())
...
def render(kind):
    ...
    def part(side,name,color,dz=0,dx=0):
        return actor(mesh(f'mechanical/revH/{side}-{name}.stl'),(offset+dx,0,dz),color)
    ...
    for side in sides:
        ...
        glass=vtk.vtkPlaneSource();glass.SetOrigin(offset+cx-5.65,hy-30,16.115+dz);glass.SetPoint1(offset+cx+5.65,hy-30,16.115+dz);glass.SetPoint2(offset+cx-5.65,hy-4,16.115+dz)
    ...
    titles={'assembled':'FILO36  /  REV H','top':'FILO36  /  TOP VIEW','side':'FILO36  /  SIDE PROFILE','stack':'FILO36  /  REMOVABLE STACK','detail':'FILO36  /  CONTINUOUS INNER EDGE'}
    label(titles[kind],105,1380,44)
    sub=('Left half  /  KLP LAME  /  Orthographic profile' if kind=='side' else '36 keys  /  KLP LAME  /  Two nice!view displays  /  24 mm bay') if kind!='stack' else 'Recessed battery. Removable controller, display and opaque frame.'
    sub='Straight case flank / Original thumb angle / R1.2 corners' if kind=='detail' else sub
    label(sub,108,1334,25,(.37,.44,.41))
    label('RevH CAD study. Wiring, final connectors, fit and operation remain untested.',108,60,24,(.36,.42,.39))
    ...
    writer=vtk.vtkPNGWriter();writer.SetFileName(str(IMAGE_DIR/f'revH-{kind}.png'));writer.SetInputConnection(f.GetOutputPort());writer.Write();win.Finalize()
    views[kind]={'keys':18*len(sides),'halves':sides,'exploded':kind=='stack','image_sha256':hashlib.sha256((IMAGE_DIR/f'revH-{kind}.png').read_bytes()).hexdigest()}
    print(kind,'rendered',flush=True)


for kind in ['assembled','top','side','stack','detail']:render(kind)
receipt={'revision':'H','model_sha256':hashlib.sha256((ROOT/'design/revH.json').read_bytes()).hexdigest(),
         ...
         'illustrative':['component envelopes','switch bodies','screw heads','feet','screen artwork','unmeasured cap stem-tip datum Z=11.7'],
         'unresolved':['battery cable routing','routed PCB','exact sockets and connectors','physical fit and tests']}
(ROOT/'validation/revH-render.json').write_text(json.dumps(receipt,indent=2)+'\n')
