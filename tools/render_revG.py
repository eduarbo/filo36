#!/usr/bin/env python3
"""Render generated revG CAD and unchanged KLP meshes. No retouching or geometry edits.
...
def render(kind):
    ...
    def mesh(path):
        if path not in cache:
            ...
            if path.startswith('mechanical/revG/'):
    ...
    def part(side,name,color,dz=0,dx=0):
        return actor(mesh(f'mechanical/revG/{side}-{name}.stl'),(offset+dx,0,dz),color)
    ...
    titles={'assembled':'FILO36  /  REV G','top':'FILO36  /  TOP VIEW','side':'FILO36  /  SIDE PROFILE','stack':'FILO36  /  REMOVABLE STACK'}
    ...
    label('RevG CAD study. Wiring, final connectors, fit and operation remain untested.',108,60,24,(.36,.42,.39))
    ...
    writer=vtk.vtkPNGWriter();writer.SetFileName(str(IMAGE_DIR/f'revG-{kind}.png'));writer.SetInputConnection(f.GetOutputPort());writer.Write();win.Finalize()
    views[kind]={'keys':18*len(sides),'halves':sides,'exploded':kind=='stack','image_sha256':hashlib.sha256((IMAGE_DIR/f'revG-{kind}.png').read_bytes()).hexdigest()}
    print(kind,'rendered',flush=True)


for kind in ['assembled','top','side','stack']:render(kind)
receipt={'revision':'G','model_sha256':hashlib.sha256((ROOT/'design/revG.json').read_bytes()).hexdigest(),
...
(ROOT/'validation/revG-render.json').write_text(json.dumps(receipt,indent=2)+'\n')
