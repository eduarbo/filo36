#!/usr/bin/env python3
"""Pack unchanged revG meshes for the interactive viewer. Millimetres throughout.
...
model = json.loads((ROOT/'design/revG.json').read_text())
layout = json.loads((ROOT/'design/layout.json').read_text())
scene = {'revision':'G', 'units':'mm', 'geometries':{}, 'parts':[], 'sources':[]}
...
def mesh(path):
    ...
    indices=cells[:,[1,2,3] if path.startswith('mechanical/revG/') else [1,3,2]].reshape(-1)
...
for side,keys in layout['halves'].items():
    ...
    for name,(label,group,color,explode) in groups.items():
        add(label,side,group,color,[offset,0,0],explode,mesh(f'mechanical/revG/{side}-{name}.stl'))
    for i in range(1,6):
        add(f'Washer {i}',side,'fasteners','#89918a',[offset,0,0],9,mesh(f'mechanical/revG/{side}-washer-{i}.stl'))
    for style in catalog['frame_styles']:mesh(f'mechanical/revG/{side}-frame-{style}.stl')
    for key in keys:
...
for path in ['design/revG.json','design/layout.json','tools/build_viewer_revG.py','keycaps/catalog.json']:
    scene['sources'].append({'path':path,'sha256':digest(path)})
