#!/usr/bin/env python3
"""Render generated revF CAD and unchanged KLP meshes. No retouching or geometry edits.
...
m=json.loads((ROOT/'design/revF.json').read_text())
...
def render(kind):
    ...
    def mesh(path):
        if path not in cache:
            ...
            if path.startswith('mechanical/revF/'):
    ...
    def part(side,name,color,dz=0,dx=0):
        return actor(mesh(f'mechanical/revF/{side}-{name}.stl'),(offset+dx,0,dz),color)
    ...
    titles={'assembled':'FILO36  /  REV F','top':'FILO36  /  VISTA SUPERIOR','side':'FILO36  /  PERFIL LATERAL','stack':'FILO36  /  STACK DESMONTABLE'}
    label(titles[kind],105,1380,44)
    sub=('Mitad izquierda  /  KLP LAME  /  Perfil ortografico' if kind=='side' else '36 teclas  /  KLP LAME  /  Dos nice!view  /  Bahia de 24 mm') if kind!='stack' else 'Bateria rebajada en PCB. Micro y pantalla extraibles. Marco abierto.'
    label(sub,108,1334,25,(.37,.44,.41))
    label('Estudio CAD revF. Cables, conectores definitivos, encaje y funcionamiento pendientes.',108,60,24,(.36,.42,.39))
    ...
    if kind=='side':
        label('Plate: 7.6 mm',105,1235,29);label('Cubierta electronica: 16.6 mm',105,1190,29)
    ...
    if kind=='stack':
        ...
        label('PCB con abertura de bateria; sin ruteo',105,170,24,(.37,.44,.41))
    win.Render();f=vtk.vtkWindowToImageFilter();f.SetInput(win);f.SetInputBufferTypeToRGB();f.ReadFrontBufferOff();f.Update()
    writer=vtk.vtkPNGWriter();writer.SetFileName(str(IMAGE_DIR/f'revF-{kind}.png'));writer.SetInputConnection(f.GetOutputPort());writer.Write();win.Finalize()
    views[kind]={'keys':18*len(sides),'halves':sides,'exploded':kind=='stack','image_sha256':hashlib.sha256((IMAGE_DIR/f'revF-{kind}.png').read_bytes()).hexdigest()}
    print(kind,'rendered',flush=True)


for kind in ['assembled','top','side','stack']:render(kind)
receipt={'revision':'F','model_sha256':hashlib.sha256((ROOT/'design/revF.json').read_bytes()).hexdigest(),
...
(ROOT/'validation/revF-render.json').write_text(json.dumps(receipt,indent=2)+'\n')
