...
Open the native source and expand **Construction**. Duplicate a supplied theme, then edit the `Theme_*` boxes/cylinders or the frame section sketches. Preserve the cavity, mounting bosses, screen window and service cuts. The case outline is a native sketch with **20 intentional corners and analytical R0.8 arcs**; mesh tessellation does not add design corners. Remove only the Block constraints you intend to edit.
