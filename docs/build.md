...
The latest [support study](slim-mount-study.md) finds that lifting the display and its sled together crosses the modeled battery leads during the first 0.5–2 mm. That route is not accepted for assembly instructions: lead routing or the release sequence must be corrected and checked before a physical build. The conflict affects the reference stack as well as the lower candidates.
